...
class MBFStateMachine(smach.StateMachine):
    ...
    recovery_behavior_index = 0

    @classmethod
    def set_recovery_behaviors(self, recovery_behaviors):
        _recovery_behaviors = recovery_behaviors
    ...
    def __init__(self):
        ...
        if _recovery_behaviors is None:
    ...
    @cb_interface(outcomes=['outcome','message'],input_keys=['recovery_behavior_index'], output_keys=['recovery_behavior_index'])
    def recovery_goal_cb(self, userdata, goal):
        ...
        _recovery_behaviors = [entry['name'] for entry in rospy.get_param('/move_base_flex/recovery_behaviors')]
        
        behavior = _recovery_behaviors[userdata.recovery_behavior_index]
        print 'RECOVERY BEHAVIOR:', behavior
        goal.behavior = behavior
        userdata.recovery_behavior_index += 1

        if userdata.recovery_behavior_index >= len(_recovery_behaviors):
